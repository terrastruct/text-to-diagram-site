// Auto-generated file - do not edit manually
// Generated on: 2025-09-16T13:04:56.595Z
// This file contains all image imports from the examples render directories

import d2_basic_dagre from '@/examples/0_basic/render/d2_dagre.svg';
import d2_basic_elk from '@/examples/0_basic/render/d2_elk.svg';
import d2_basic_tala from '@/examples/0_basic/render/d2_tala.svg';
import graphviz_basic_dot from '@/examples/0_basic/render/graphviz_dot.svg';
import graphviz_basic_neato from '@/examples/0_basic/render/graphviz_neato.svg';
import mermaid_basic_dagre from '@/examples/0_basic/render/mermaid_dagre.svg';
import mermaid_basic_elk from '@/examples/0_basic/render/mermaid_elk.svg';
import plantuml_basic_dot from '@/examples/0_basic/render/plantuml_dot.svg';

import d2_containers_dagre from '@/examples/1_containers/render/d2_dagre.svg';
import d2_containers_elk from '@/examples/1_containers/render/d2_elk.svg';
import d2_containers_tala from '@/examples/1_containers/render/d2_tala.svg';
import graphviz_containers_dot from '@/examples/1_containers/render/graphviz_dot.svg';
import graphviz_containers_neato from '@/examples/1_containers/render/graphviz_neato.svg';
import mermaid_containers_dagre from '@/examples/1_containers/render/mermaid_dagre.svg';
import mermaid_containers_elk from '@/examples/1_containers/render/mermaid_elk.svg';
import plantuml_containers_dot from '@/examples/1_containers/render/plantuml_dot.svg';

import d2_shapes_dagre from '@/examples/2_shapes/render/d2_dagre.svg';
import d2_shapes_elk from '@/examples/2_shapes/render/d2_elk.svg';
import d2_shapes_tala from '@/examples/2_shapes/render/d2_tala.svg';
import graphviz_shapes_dot from '@/examples/2_shapes/render/graphviz_dot.svg';
import graphviz_shapes_neato from '@/examples/2_shapes/render/graphviz_neato.svg';
import mermaid_shapes_dagre from '@/examples/2_shapes/render/mermaid_dagre.svg';
import mermaid_shapes_elk from '@/examples/2_shapes/render/mermaid_elk.svg';
import plantuml_shapes_dot from '@/examples/2_shapes/render/plantuml_dot.svg';

import d2_trees_dagre from '@/examples/3_trees/render/d2_dagre.svg';
import d2_trees_elk from '@/examples/3_trees/render/d2_elk.svg';
import d2_trees_tala from '@/examples/3_trees/render/d2_tala.svg';
import graphviz_trees_dot from '@/examples/3_trees/render/graphviz_dot.svg';
import graphviz_trees_neato from '@/examples/3_trees/render/graphviz_neato.svg';
import mermaid_trees_dagre from '@/examples/3_trees/render/mermaid_dagre.svg';
import mermaid_trees_elk from '@/examples/3_trees/render/mermaid_elk.svg';
import plantuml_trees_dot from '@/examples/3_trees/render/plantuml_dot.svg';

import d2_chess_dagre from '@/examples/4_chess/render/d2_dagre.svg';
import d2_chess_elk from '@/examples/4_chess/render/d2_elk.svg';
import d2_chess_tala from '@/examples/4_chess/render/d2_tala.svg';
import graphviz_chess_dot from '@/examples/4_chess/render/graphviz_dot.svg';
import graphviz_chess_neato from '@/examples/4_chess/render/graphviz_neato.svg';
import mermaid_chess_dagre from '@/examples/4_chess/render/mermaid_dagre.svg';
import mermaid_chess_elk from '@/examples/4_chess/render/mermaid_elk.svg';
import plantuml_chess_dot from '@/examples/4_chess/render/plantuml_dot.svg';

import d2_icons_dagre from '@/examples/5_icons/render/d2_dagre.svg';
import d2_icons_elk from '@/examples/5_icons/render/d2_elk.svg';
import d2_icons_tala from '@/examples/5_icons/render/d2_tala.svg';
import graphviz_icons_dot from '@/examples/5_icons/render/graphviz_dot.svg';
import graphviz_icons_neato from '@/examples/5_icons/render/graphviz_neato.svg';
import mermaid_icons_dagre from '@/examples/5_icons/render/mermaid_dagre.svg';
import mermaid_icons_elk from '@/examples/5_icons/render/mermaid_elk.svg';
import plantuml_icons_dot from '@/examples/5_icons/render/plantuml_dot.svg';

import d2_tables_dagre from '@/examples/6_tables/render/d2_dagre.svg';
import d2_tables_elk from '@/examples/6_tables/render/d2_elk.svg';
import d2_tables_tala from '@/examples/6_tables/render/d2_tala.svg';
import graphviz_tables_dot from '@/examples/6_tables/render/graphviz_dot.svg';
import graphviz_tables_neato from '@/examples/6_tables/render/graphviz_neato.svg';
import mermaid_tables_dagre from '@/examples/6_tables/render/mermaid_dagre.svg';
import mermaid_tables_elk from '@/examples/6_tables/render/mermaid_elk.svg';
import plantuml_tables_dot from '@/examples/6_tables/render/plantuml_dot.svg';

import d2_sequence_dagre from '@/examples/7_sequence/render/d2_dagre.svg';
import d2_sequence_elk from '@/examples/7_sequence/render/d2_elk.svg';
import d2_sequence_tala from '@/examples/7_sequence/render/d2_tala.svg';
import mermaid_sequence_dagre from '@/examples/7_sequence/render/mermaid_dagre.svg';
import mermaid_sequence_elk from '@/examples/7_sequence/render/mermaid_elk.svg';
import plantuml_sequence_dot from '@/examples/7_sequence/render/plantuml_dot.svg';

import d2_text_dagre from '@/examples/8_text/render/d2_dagre.svg';
import d2_text_elk from '@/examples/8_text/render/d2_elk.svg';
import d2_text_tala from '@/examples/8_text/render/d2_tala.svg';
import mermaid_text_dagre from '@/examples/8_text/render/mermaid_dagre.svg';
import mermaid_text_elk from '@/examples/8_text/render/mermaid_elk.svg';
import plantuml_text_dot from '@/examples/8_text/render/plantuml_dot.svg';

import d2_code_dagre from '@/examples/9_code/render/d2_dagre.svg';
import d2_code_elk from '@/examples/9_code/render/d2_elk.svg';
import d2_code_tala from '@/examples/9_code/render/d2_tala.svg';
import graphviz_code_dot from '@/examples/9_code/render/graphviz_dot.svg';
import graphviz_code_neato from '@/examples/9_code/render/graphviz_neato.svg';

import d2_wiiu_dagre from '@/examples/10_wiiu/render/d2_dagre.svg';
import d2_wiiu_elk from '@/examples/10_wiiu/render/d2_elk.svg';
import d2_wiiu_tala from '@/examples/10_wiiu/render/d2_tala.svg';
import graphviz_wiiu_dot from '@/examples/10_wiiu/render/graphviz_dot.svg';
import graphviz_wiiu_neato from '@/examples/10_wiiu/render/graphviz_neato.svg';
import mermaid_wiiu_dagre from '@/examples/10_wiiu/render/mermaid_dagre.svg';
import mermaid_wiiu_elk from '@/examples/10_wiiu/render/mermaid_elk.svg';
import plantuml_wiiu_dot from '@/examples/10_wiiu/render/plantuml_dot.svg';

import d2_latex_dagre from '@/examples/11_latex/render/d2_dagre.svg';
import d2_latex_elk from '@/examples/11_latex/render/d2_elk.svg';
import d2_latex_tala from '@/examples/11_latex/render/d2_tala.svg';
import mermaid_latex_dagre from '@/examples/11_latex/render/mermaid_dagre.svg';
import mermaid_latex_elk from '@/examples/11_latex/render/mermaid_elk.svg';

import d2_class_dagre from '@/examples/12_class/render/d2_dagre.svg';
import d2_class_elk from '@/examples/12_class/render/d2_elk.svg';
import d2_class_tala from '@/examples/12_class/render/d2_tala.svg';
import graphviz_class_dot from '@/examples/12_class/render/graphviz_dot.svg';
import graphviz_class_neato from '@/examples/12_class/render/graphviz_neato.svg';
import mermaid_class_dagre from '@/examples/12_class/render/mermaid_dagre.svg';
import mermaid_class_elk from '@/examples/12_class/render/mermaid_elk.svg';
import plantuml_class_dot from '@/examples/12_class/render/plantuml_dot.svg';

import d2_state_dagre from '@/examples/13_state/render/d2_dagre.svg';
import d2_state_elk from '@/examples/13_state/render/d2_elk.svg';
import d2_state_tala from '@/examples/13_state/render/d2_tala.svg';
import graphviz_state_dot from '@/examples/13_state/render/graphviz_dot.svg';
import graphviz_state_neato from '@/examples/13_state/render/graphviz_neato.svg';
import mermaid_state_dagre from '@/examples/13_state/render/mermaid_dagre.svg';
import mermaid_state_elk from '@/examples/13_state/render/mermaid_elk.svg';
import plantuml_state_dot from '@/examples/13_state/render/plantuml_dot.svg';

export function getImage(id: string) {
  switch (id.toLowerCase()) {
    case 'd2-basic-dagre':
      return d2_basic_dagre;
    case 'd2-basic-elk':
      return d2_basic_elk;
    case 'd2-basic-tala':
      return d2_basic_tala;
    case 'graphviz-basic-dot':
      return graphviz_basic_dot;
    case 'graphviz-basic-neato':
      return graphviz_basic_neato;
    case 'mermaid-basic-dagre':
      return mermaid_basic_dagre;
    case 'mermaid-basic-elk':
      return mermaid_basic_elk;
    case 'plantuml-basic-dot':
      return plantuml_basic_dot;

    case 'd2-containers-dagre':
      return d2_containers_dagre;
    case 'd2-containers-elk':
      return d2_containers_elk;
    case 'd2-containers-tala':
      return d2_containers_tala;
    case 'graphviz-containers-dot':
      return graphviz_containers_dot;
    case 'graphviz-containers-neato':
      return graphviz_containers_neato;
    case 'mermaid-containers-dagre':
      return mermaid_containers_dagre;
    case 'mermaid-containers-elk':
      return mermaid_containers_elk;
    case 'plantuml-containers-dot':
      return plantuml_containers_dot;

    case 'd2-shapes-dagre':
      return d2_shapes_dagre;
    case 'd2-shapes-elk':
      return d2_shapes_elk;
    case 'd2-shapes-tala':
      return d2_shapes_tala;
    case 'graphviz-shapes-dot':
      return graphviz_shapes_dot;
    case 'graphviz-shapes-neato':
      return graphviz_shapes_neato;
    case 'mermaid-shapes-dagre':
      return mermaid_shapes_dagre;
    case 'mermaid-shapes-elk':
      return mermaid_shapes_elk;
    case 'plantuml-shapes-dot':
      return plantuml_shapes_dot;

    case 'd2-trees-dagre':
      return d2_trees_dagre;
    case 'd2-trees-elk':
      return d2_trees_elk;
    case 'd2-trees-tala':
      return d2_trees_tala;
    case 'graphviz-trees-dot':
      return graphviz_trees_dot;
    case 'graphviz-trees-neato':
      return graphviz_trees_neato;
    case 'mermaid-trees-dagre':
      return mermaid_trees_dagre;
    case 'mermaid-trees-elk':
      return mermaid_trees_elk;
    case 'plantuml-trees-dot':
      return plantuml_trees_dot;

    case 'd2-chess-dagre':
      return d2_chess_dagre;
    case 'd2-chess-elk':
      return d2_chess_elk;
    case 'd2-chess-tala':
      return d2_chess_tala;
    case 'graphviz-chess-dot':
      return graphviz_chess_dot;
    case 'graphviz-chess-neato':
      return graphviz_chess_neato;
    case 'mermaid-chess-dagre':
      return mermaid_chess_dagre;
    case 'mermaid-chess-elk':
      return mermaid_chess_elk;
    case 'plantuml-chess-dot':
      return plantuml_chess_dot;

    case 'd2-icons-dagre':
      return d2_icons_dagre;
    case 'd2-icons-elk':
      return d2_icons_elk;
    case 'd2-icons-tala':
      return d2_icons_tala;
    case 'graphviz-icons-dot':
      return graphviz_icons_dot;
    case 'graphviz-icons-neato':
      return graphviz_icons_neato;
    case 'mermaid-icons-dagre':
      return mermaid_icons_dagre;
    case 'mermaid-icons-elk':
      return mermaid_icons_elk;
    case 'plantuml-icons-dot':
      return plantuml_icons_dot;

    case 'd2-tables-dagre':
      return d2_tables_dagre;
    case 'd2-tables-elk':
      return d2_tables_elk;
    case 'd2-tables-tala':
      return d2_tables_tala;
    case 'graphviz-tables-dot':
      return graphviz_tables_dot;
    case 'graphviz-tables-neato':
      return graphviz_tables_neato;
    case 'mermaid-tables-dagre':
      return mermaid_tables_dagre;
    case 'mermaid-tables-elk':
      return mermaid_tables_elk;
    case 'plantuml-tables-dot':
      return plantuml_tables_dot;

    case 'd2-sequence-dagre':
      return d2_sequence_dagre;
    case 'd2-sequence-elk':
      return d2_sequence_elk;
    case 'd2-sequence-tala':
      return d2_sequence_tala;
    case 'mermaid-sequence-dagre':
      return mermaid_sequence_dagre;
    case 'mermaid-sequence-elk':
      return mermaid_sequence_elk;
    case 'plantuml-sequence-dot':
      return plantuml_sequence_dot;

    case 'd2-text-dagre':
      return d2_text_dagre;
    case 'd2-text-elk':
      return d2_text_elk;
    case 'd2-text-tala':
      return d2_text_tala;
    case 'mermaid-text-dagre':
      return mermaid_text_dagre;
    case 'mermaid-text-elk':
      return mermaid_text_elk;
    case 'plantuml-text-dot':
      return plantuml_text_dot;

    case 'd2-code-dagre':
      return d2_code_dagre;
    case 'd2-code-elk':
      return d2_code_elk;
    case 'd2-code-tala':
      return d2_code_tala;
    case 'graphviz-code-dot':
      return graphviz_code_dot;
    case 'graphviz-code-neato':
      return graphviz_code_neato;

    case 'd2-wiiu-dagre':
      return d2_wiiu_dagre;
    case 'd2-wiiu-elk':
      return d2_wiiu_elk;
    case 'd2-wiiu-tala':
      return d2_wiiu_tala;
    case 'graphviz-wiiu-dot':
      return graphviz_wiiu_dot;
    case 'graphviz-wiiu-neato':
      return graphviz_wiiu_neato;
    case 'mermaid-wiiu-dagre':
      return mermaid_wiiu_dagre;
    case 'mermaid-wiiu-elk':
      return mermaid_wiiu_elk;
    case 'plantuml-wiiu-dot':
      return plantuml_wiiu_dot;

    case 'd2-latex-dagre':
      return d2_latex_dagre;
    case 'd2-latex-elk':
      return d2_latex_elk;
    case 'd2-latex-tala':
      return d2_latex_tala;
    case 'mermaid-latex-dagre':
      return mermaid_latex_dagre;
    case 'mermaid-latex-elk':
      return mermaid_latex_elk;

    case 'd2-class-dagre':
      return d2_class_dagre;
    case 'd2-class-elk':
      return d2_class_elk;
    case 'd2-class-tala':
      return d2_class_tala;
    case 'graphviz-class-dot':
      return graphviz_class_dot;
    case 'graphviz-class-neato':
      return graphviz_class_neato;
    case 'mermaid-class-dagre':
      return mermaid_class_dagre;
    case 'mermaid-class-elk':
      return mermaid_class_elk;
    case 'plantuml-class-dot':
      return plantuml_class_dot;

    case 'd2-state-dagre':
      return d2_state_dagre;
    case 'd2-state-elk':
      return d2_state_elk;
    case 'd2-state-tala':
      return d2_state_tala;
    case 'graphviz-state-dot':
      return graphviz_state_dot;
    case 'graphviz-state-neato':
      return graphviz_state_neato;
    case 'mermaid-state-dagre':
      return mermaid_state_dagre;
    case 'mermaid-state-elk':
      return mermaid_state_elk;
    case 'plantuml-state-dot':
      return plantuml_state_dot;
  }
}
